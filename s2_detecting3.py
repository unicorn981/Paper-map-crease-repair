...
def shadowget(ROI_cube,filepath):
    ...
    src = cv2.dilate(src, kernel1 , iterations = 9)
    result= cv2.erode(src, kernel2, iterations = 9)
    # 图像闭运算
...
def find_max_region(mask_sel,filepath):
...
def Optimize(dst,filepath): #输入二值化折痕后图像
    New_Img = dst.copy()
...
def Rosenfeld(im, filepath):
    ...
    #
    #
    # skel = find_max_region(skel,filepath)
